...
Source100: http://www.linux-pam.org/documentation/Linux-PAM-1.1.8-docs.tar.bz2
...
Provides: /sbin/pam_tally2
Provides: /sbin/pam_timestamp_check
Provides: /sbin/faillock
Provides: /sbin/unix_chkpwd
Provides: /sbin/unix_update
Provides: /sbin/mkhomedir_helper
Provides: /sbin/pwhistory_helper
Conflicts: filesystem < 3
%else
%define _pamlibdir /%{_lib}
%define _pamsbindir /sbin
%endif
%define _moduledir %{_pamlibdir}/security
...
BuildRequires: bison, flex, flex-static, sed
...
%if %{with YPRPC}
## pam_access and pam_unix use these for NIS RPC functionality
BuildRequires: libtirpc-devel, libgssglue-devel
%endif
...
%prep
%setup -q -n Linux-PAM-%{version} -a 2 -b 100
...
for f in \
    other config-util postlogin \
    fingerprint-auth password-auth smartcard-auth system-auth ; do
  install -m 644 .%{_pamconfdir}/${f}.pamd $RPM_BUILD_ROOT%{_pamconfdir}/${f}
done
# We do not currently ship 20-nproc, but if we add it back it should be renamed
# to 90-nproc for the benefit of customers who modified it under the old name.
...
for lib in libpam libpamc libpam_misc ; do
   libf=$(basename $RPM_BUILD_ROOT%{_pamlibdir}/${lib}.so.*.*)
   ln -sf %{_pamlibdir}/${libf} $RPM_BUILD_ROOT%{_libdir}/${lib}.so
done
%endif
...
%if %{without usrmerge}
# add some compat links for the changed file locations
install -d %{buildroot}%{_sbindir}
for f in pam_console_apply pam_tally2 pam_timestamp_check \
         faillock unix_chkpwd unix_update mkhomedir_helper pwhistory_helper ; do
     ln -sf /sbin/$f %{buildroot}%{_sbindir}/$f
done
%endif
...
for module in $RPM_BUILD_ROOT%{_moduledir}/pam*.so ; do
	if ! env LD_LIBRARY_PATH=$RPM_BUILD_ROOT%{_pamlibdir} \
		 %{SOURCE11} -L$RPM_BUILD_ROOT%{_pamlibdir} -ldl -lpam ${module} ; then
...
%{_pamsbindir}/pam_console_apply
%{_pamsbindir}/pam_tally2
%{_pamsbindir}/faillock
%attr(4755,root,root) %{_pamsbindir}/pam_timestamp_check
%attr(4755,root,root) %{_pamsbindir}/unix_chkpwd
%attr(0700,root,root) %{_pamsbindir}/unix_update
%attr(0755,root,root) %{_pamsbindir}/mkhomedir_helper
%attr(0755,root,root) %{_pamsbindir}/pwhistory_helper
...
%if "%{_pamlibdir}" != "%{_libdir}"
# these are needed for make check to work but we only need to package the links in _libdir
%exclude %{_pamlibdir}/libpam.so
%exclude %{_pamlibdir}/libpamc.so
%exclude %{_pamlibdir}/libpam_misc.so
%endif
%doc doc/mwg/*.txt doc/mwg/html
%doc doc/adg/*.txt doc/adg/html

%changelog
* Thu Aug 20 2015 Amazon Linux AMI <amazon-linux-ami@amazon.com>
- import source package EL7/pam-1.1.8-12.el7_1.1
...
- import source package EL7/pam-1.1.8-12.el7
...
* Mon Aug 25 2014 Ben Cressey <bcressey@amazon.com>
- conditionalize use of systemd module

* Mon Aug 25 2014 Tomáš Mráz <tmraz@redhat.com> 1.1.8-10
- pam_lastlog: fix uninitialized access of parts of lastlog structure

* Tue Jun 10 2014 Andrew Jorgensen <ajorgens@amazon.com>
- Raise nproc to 7168, but keep it in 90-nproc.conf

* Wed May 14 2014 Cristian Gafton <gafton@amazon.com>
- restore user helper paths to their pre-usrmerge state
- document build requires needed for NIS RPC functionality

* Wed May 7 2014 Cristian Gafton <gafton@amazon.com>
- import source package RHEL7/pam-1.1.8-9.el7
...
* Sun Mar 16 2014 Cristian Gafton <gafton@amazon.com>
- run make check against freshly built libs instead of system-installed ones
- create devel .so links to correct libraries during build

* Sat Mar 15 2014 Cristian Gafton <gafton@amazon.com>
- add bcond for usrmerge
...
- import source package RHEL6/pam-1.1.1-13.el6
...
* Fri Aug 24 2012 Tom Kirchner <tjk@amazon.com>
- Add flex-static to BuildRequires after split from flex
...
* Fri Apr 20 2012 Cristian Gafton <gafton@amazon.com>
- import source package RHEL6/pam-1.1.1-10.el6_2.1
...
- import source package RHEL6/pam-1.1.1-10.el6
...
- re-enable manpage regeneration
...
- import source package RHEL6/pam-1.1.1-8.el6
...
- import source package RHEL6/pam-1.1.1-4.el6_0.1
...
- add offline source documentation use rpm's build conditionals
- run autoconf in build instead of prep
- update to use rpm's support of build conditionals

* Tue Jul 13 2010 Cristian Gafton <gafton@amazon.com> - 1.1.1-4.1
- add offline docuemntation so we don't need to run firefox to build the thing

* Fri Jul 9 2010 Cristian Gafton <gafton@amazon.com>
- import source package RHEL6/pam-1.1.1-4.el6
- import source package RHEL6/pam-1.1.1-2.el6

* Fri May 7 2010 Cristian Gafton <gafton@amazon.com>
- import source package RHEL5/pam-0.99.6.2-6.el5_4.1
- import source package RHEL5/pam-0.99.6.2-6.el5
- import source package RHEL5/pam-0.99.6.2-4.el5
- import source package RHEL5/pam-0.99.6.2-3.27.el5
- import source package RHEL5/pam-0.99.6.2-3.26.el5
- import source package RHEL5/pam-0.99.6.2-3.14.el5
- added submodule prep for package pam
